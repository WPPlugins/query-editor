...
function query_editor_init() {
	...
	// taxonomy ordering
	add_settings_field( 'qe_orderby_tax', __( 'Order by taxonomy:' ), 'query_editor_orderby_tax', $page, $section, get_option( 'qe_orderby_tax' ) );
	add_settings_field( 'qe_orderby_tax_order', __( 'Term order:' ), 'query_editor_orderby_tax_order', $page, $section, get_option( 'qe_orderby_tax_order', 'asc' ) );
...
function query_editor_orderby_tax_order( $order ) { ?>
	<label for="qe_orderby_tax_order_asc"><input <?php checked( 'asc', $order ); ?> id="qe_orderby_tax_order_asc" type="radio" name="qe_orderby_tax_order" value="asc" /> <?php _e( 'Ascending' ); ?></label>
	<label for="qe_orderby_tax_order_desc"><input <?php checked( 'desc', $order ); ?> id="qe_orderby_tax_order_desc" type="radio" name="qe_orderby_tax_order" value="desc" /> <?php _e( 'Descending' ); ?></label>
	<?php
}

function query_editor_orderby_tax( $orderby ) { ?>
	<select name="qe_orderby_tax">
		<option value=""><?php _e( 'None' ); ?></option>
		<?php foreach( get_taxonomies( array(), 'objects' ) as $taxonomy ) { ?>
		<option <?php selected( $taxonomy->name, $orderby ); ?> value="<?php esc_attr_e( $taxonomy->name ); ?>"><?php _e( $taxonomy->labels->name ); ?></option>
		<?php } ?>
	</select>
	<p class="description">Use this field to order posats by the terms in a taxonomy.</p>
	<?php
}
...
function save_query_editor() {
	...
	get_option( 'qe_orderby_tax_order', 'asc' );
	update_option( 'qe_orderby_tax_order', sanitize_key( $_POST[ 'qe_orderby_tax_order' ] ) );

	get_option( 'qe_orderby_tax', 'date' );
	update_option( 'qe_orderby_tax', in_array( $_POST[ 'qe_orderby_tax' ], get_taxonomies() ) ? sanitize_text_field( $_POST[ 'qe_orderby_tax' ] ) : false );
...
// code form scribu and mike schinkel
add_filter( 'posts_clauses', 'query_editor_orderby_taxonomy', 10, 2 );
function query_editor_orderby_taxonomy( $clauses, $wp_query ) {
	global $wpdb;
...
function exclude_taxonomies_filter( $query ) {
	...
	// no sense in doing anything singular pages
